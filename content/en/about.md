...
An open and lightweight repair wiki based on Hugo / Docsy. For now it only holds my own articles — the same ones I also post on [logi.wiki](https://logi.wiki) and [repair.wiki](https://repair.wiki). If it works out, it will grow into a bigger, public project.

## On AI use

Yes, i use AI (Claude) to draft, spell-check and format these articles — no secret there. And, for now (lol), i still correct way more technical mistakes from Claude than it finds spelling errors in my text.

To keep the assistant honest, i run it through a strict skill file that acts as a filter: it may only write facts i stated or facts it researched and cited — no plausible-sounding padding. That exact file (`.claude/skills/grounded-wiki-editing/SKILL.md`) is quoted verbatim below.

## The skill i use as a filter
